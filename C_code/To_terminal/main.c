...
loop_count = 0;
	...
	{	
		...
		*(uint32_t *)h2p_lw_led_addr = DataOut;
		...
		printf("\nTIME_SINCE_LAST_PULSER_FALLING_EDGE(us) VOLTAGE(mV)\n");
	...
	{
		printf("NUMBER TRIGGER_OFFSET(us) TIME_KTM_WOULD_HAVE_TRIGGERED(us) BEAM_ON_MEAN_VOLTAGE(mv) RESIDUAL_BEAM_MEAN_VOLTAGE(mv)\n");
		DataOut = 4;
		*(uint32_t *)h2p_lw_led_addr = DataOut;
